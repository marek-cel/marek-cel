...
{
    createLight();
    createSphere(osg::Vec3(0.0, 0.0, 0.0));
}
...
void SceneRoot::createSphere(osg::Vec3 pos)
{
    ...
    osg::ref_ptr<osg::Texture2D> texture1 = readTextureFromFile("../../../data/world_oceanmask_1024.jpg");
    if ( texture1.valid() )
    {
        stateSet->setTextureAttributeAndModes(1, texture1, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
    }

    osg::ref_ptr<osg::Texture2D> texture2 = readTextureFromFile("../../../data/cloud_combined_1024.jpg");
    if ( texture2.valid() )
    {
