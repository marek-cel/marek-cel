...
double CAAElementsPlanetaryOrbit::MercuryMeanLongitude(double JD) noexcept
{
...
double CAAElementsPlanetaryOrbit::MercuryInclination(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MercuryLongitudeAscendingNode(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MercuryLongitudePerihelion(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::VenusMeanLongitude(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::VenusInclination(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::VenusLongitudeAscendingNode(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::VenusLongitudePerihelion(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::EarthMeanLongitude(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MarsInclination(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MarsLongitudeAscendingNode(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MarsLongitudePerihelion(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::JupiterMeanLongitude(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::JupiterInclination(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::JupiterLongitudeAscendingNode(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::JupiterLongitudePerihelion(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::SaturnMeanLongitude(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::SaturnInclination(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::SaturnLongitudeAscendingNode(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::SaturnLongitudePerihelion(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::UranusMeanLongitude(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::UranusInclination(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::UranusLongitudeAscendingNode(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::UranusLongitudePerihelion(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::NeptuneMeanLongitude(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::NeptuneInclination(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::NeptuneLongitudeAscendingNode(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::NeptuneLongitudePerihelion(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MercuryMeanLongitudeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MercuryInclinationJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MercuryLongitudeAscendingNodeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MercuryLongitudePerihelionJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::VenusMeanLongitudeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::VenusInclinationJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::VenusLongitudeAscendingNodeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::VenusLongitudePerihelionJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::EarthMeanLongitudeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::EarthLongitudeAscendingNodeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MarsMeanLongitudeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MarsInclinationJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MarsLongitudeAscendingNodeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::MarsLongitudePerihelionJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::JupiterMeanLongitudeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::JupiterInclinationJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::JupiterLongitudeAscendingNodeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::JupiterLongitudePerihelionJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::SaturnMeanLongitudeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::SaturnInclinationJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::SaturnLongitudeAscendingNodeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::SaturnLongitudePerihelionJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::UranusMeanLongitudeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::UranusInclinationJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::UranusLongitudeAscendingNodeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::UranusLongitudePerihelionJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::NeptuneMeanLongitudeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::NeptuneInclinationJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
...
double CAAElementsPlanetaryOrbit::NeptuneLongitudeAscendingNodeJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
  const double Tcubed{Tsquared*T};
...
double CAAElementsPlanetaryOrbit::NeptuneLongitudePerihelionJ2000(double JD) noexcept
{
  const double T{(JD - 2451545)/36525};
  const double Tsquared{T*T};
