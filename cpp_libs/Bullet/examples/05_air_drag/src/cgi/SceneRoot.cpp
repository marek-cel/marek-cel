...
//    }
    ...
    std::cout << _bodySphere->getLinearVelocity().getZ() << std::endl;

    double cx = 0.5;
    double s = M_PI * pow( 1.0, 2.0 );
    double rho = 1.225;

    btVector3 drag;
    drag.setX( -0.5 * _bodySphere->getLinearVelocity().getX() * fabs(_bodySphere->getLinearVelocity().getZ()) * rho * s * cx );
    drag.setY( -0.5 * _bodySphere->getLinearVelocity().getY() * fabs(_bodySphere->getLinearVelocity().getZ()) * rho * s * cx );
    drag.setZ( -0.5 * _bodySphere->getLinearVelocity().getZ() * fabs(_bodySphere->getLinearVelocity().getZ()) * rho * s * cx );

    _bodySphere->applyCentralForce( drag );
}
...
void SceneRoot::initPhysics()
{
    ...
    // body
    {
        ...
        startTransform.setOrigin(btVector3(0, 0, 100));
