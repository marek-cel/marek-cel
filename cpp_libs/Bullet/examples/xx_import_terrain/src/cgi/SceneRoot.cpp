...
//    }
    ...
    osg::Quat q( trans.getRotation().getX(),
...
{
    osg::ref_ptr<osg::Node> nodeGround = osgDB::readNodeFile( "../data/terrain.stl" );

    if ( nodeGround.valid() )
    {
        _root->addChild( nodeGround.get() );
        ...
        nodeGround->getOrCreateStateSet()->setAttribute( material.get() );
    }
}
...
//        GLInstanceGraphicsShape* gfxShape = LoadMeshFromSTL("../data/terrain.stl",&fileIO);
...
//        _groundColShape = new btBvhTriangleMeshShape;
    ...
    // body
    {
        _sphareColShape = new btBoxShape(btVector3(1.0,1.0,1.0));
        ...
        btVector3 localInertia(mass/6.0, mass/6.0, mass/6.0);
        if (isDynamic)
