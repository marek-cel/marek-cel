...
void SceneRoot::createScene()
{
    ...
    _overleyNode = new osgSim::OverlayNode( technique );
    _pat->addChild( _overleyNode.get() );

    _overleyNode->getOrCreateStateSet()->setTextureAttribute( 1, new osg::TexEnv( osg::TexEnv::DECAL ) );
//    _overleyNode->setOverlaySubgraph( shapefile.get() );
    _overleyNode->setOverlayTextureSizeHint( 1024 );

    _overleyNode->getOrCreateStateSet()->setMode( GL_BLEND, osg::StateAttribute::ON );
    _overleyNode->getOrCreateStateSet()->setRenderingHint( osg::StateSet::TRANSPARENT_BIN );
...
{
    osg::ref_ptr<osg::Geode> geode = new osg::Geode();
    _overleyNode->setOverlaySubgraph( geode.get() );
