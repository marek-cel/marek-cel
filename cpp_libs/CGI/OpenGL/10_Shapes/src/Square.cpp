...
Square::Square(GLuint positionAttribute, GLuint colorAttribute,
               float sideLength)
...
unsigned int Square::CreateVertexArray(Vertex*& vertices)
{
    ...
    vertices[0] = Vertex(-x0, -y0, 0.0, 1, 0, 0, 1);
    vertices[1] = Vertex( x0, -y0, 0.0, 0, 1, 0, 1);
    vertices[2] = Vertex(-x0,  y0, 0.0, 1, 1, 0, 1);
    vertices[3] = Vertex( x0,  y0, 0.0, 1, 0, 1, 1);
