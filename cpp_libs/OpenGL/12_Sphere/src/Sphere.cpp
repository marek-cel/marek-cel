...
Sphere::Sphere(GLuint positionAttribute, GLuint colorAttribute,
               float radius, uint16_t segments, uint16_t rings)
    : radius_(radius)
    , segments_(segments)
    , rings_(rings)
{
...
std::vector<Vertex> Sphere::CreateVertexVector()
{
    ...
    for ( int i_lat = 0; i_lat < rings_; i_lat++ )
    {
        ...
        for ( int i_lon = 0; i_lon < segments_ + 1; i_lon++ )
        {
            ...
            v.push_back(Vertex(x_prev, y_prev, z_prev, cr, cg, cb, 1.0));
            v.push_back(Vertex(x_next, y_next, z_next, cr, cg, cb, 1.0));
